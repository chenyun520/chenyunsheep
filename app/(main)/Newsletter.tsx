...
function UserAvatar({
  ...
  return (
    <div
      className="relative h-8 w-8 shrink-0 overflow-hidden rounded-full bg-gradient-to-br from-lime-400 to-emerald-500 ring-2 ring-zinc-100 dark:ring-zinc-800"
      style={{ zIndex }}
    >
      {user.image_url ? (
        <Image
...
function SimulatedAvatar({
  ...
  return (
    <div
      className={`relative h-8 w-8 overflow-hidden rounded-full bg-gradient-to-br ring-2 ring-zinc-100 dark:ring-zinc-800 ${gradient} shrink-0`}
      style={{ zIndex }}
    >
      <span className="flex h-full w-full items-center justify-center text-xs font-medium text-white">
        {name[0]}
    ...
    <div className="mx-auto flex max-w-4xl flex-col gap-6 md:flex-row md:gap-10">
      ...
      <motion.div
        ...
        className="relative flex flex-1 items-center
                   ...
                   bg-zinc-100
                   px-8
                   py-5 shadow-[20px_20px_60px_#bebebe,-20px_-20px_60px_#ffffff] dark:bg-zinc-800
                   dark:shadow-[20px_20px_60px_#1a1a1a,-20px_-20px_60px_#2a2a2a]"
      >
        {/* 左侧：标题和图标 */}
        <div className="flex items-center gap-3">
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-lime-100 dark:bg-lime-900/30">
        ...
        <ClerkUserStats
          ...
          render={(users, totalUsers) => {
            const memberCount = totalUsers + MEMBERS_OFFSET
                ...
                <div className="flex -space-x-2">
                  ...
                  {SIMULATED_USERS.slice(0, simCount).map((sim, i) => (
                    <SimulatedAvatar
                      ...
                      zIndex={4 - realCount - i}
                    />
                  ))}
                  <div className="relative flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-zinc-200 ring-2 ring-zinc-100 dark:bg-zinc-700 dark:ring-zinc-800">
      ...
      <motion.div
        ...
        className="relative flex flex-1 items-center justify-center
                   ...
                   bg-zinc-100
                   px-8
                   py-5 shadow-[20px_20px_60px_#bebebe,-20px_-20px_60px_#ffffff] dark:bg-zinc-800 dark:shadow-[20px_20px_60px_#1a1a1a,-20px_-20px_60px_#2a2a2a]"
      >
        <span className="whitespace-nowrap text-base font-medium text-zinc-500 dark:text-zinc-400">
          Make it
        </span>
        <TextRotate
          texts={['happen', 'right', 'fast']}
